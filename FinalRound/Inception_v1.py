...
def control_speed(angle):
    if abs(angle) >= 0 and  abs(angle) <= 8:
        speed = 150
    elif abs(angle) > 8  and  abs(angle) <= 15:
        speed = 130
    else:
        speed = 80
    return speed
...
def tracking_lane(center):
    ...
    error = center-320
    angle = PID(error, 0.32,0.0,0.055)
    speed = control_speed(angle)
    return angle, speed

def get_center(b_img):
    try:
        index_left = np.min(np.where(b_img[60,:] > 0))
        index_right = np.max(np.where(b_img[60,:] > 0))
